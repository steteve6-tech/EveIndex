...
public interface DeviceRecallRecordRepository extends JpaRepository<DeviceRecallRecord, Long> {
     ...
     * 根据风险等级和国家查找记录
     */
    List<DeviceRecallRecord> findByRiskLevelAndJdCountry(CertNewsData.RiskLevel riskLevel, String jdCountry);
     ...
     * 根据国家代码查找记录
     */
    List<DeviceRecallRecord> findByCountryCode(String countryCode);
    
    /**
     * 根据国家代码查找记录（分页）
     */
    org.springframework.data.domain.Page<DeviceRecallRecord> findByCountryCode(String countryCode, org.springframework.data.domain.Pageable pageable);
