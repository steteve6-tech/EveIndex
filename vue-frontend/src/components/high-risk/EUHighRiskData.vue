      ...
      <a-row :gutter="16">
        ...
        <a-col :span="6">
          <a-card class="stat-card">
            <a-statistic
              title="CE认证高风险"
              :value="statistics.ceHighRisk"
              :value-style="{ color: '#ff7a45' }"
            >
              <template #prefix>
                <SafetyCertificateOutlined style="color: #ff7a45" />
              </template>
            </a-statistic>
          </a-card>
        </a-col>
...
const handleRiskAdjustSubmit = async () => {
  ...
  try {
    // 从当前编辑数据中获取dataType，如果没有则默认使用'recall'
    const dataType = currentEditData.value.dataType || currentEditData.value.type || 'recall'
...
const loadStatistics = async () => {
  try {
    const result = await getHighRiskData({ countryCode: 'EU', riskLevel: 'HIGH' })
    if (result.success) {
